...
const PricingPlans = () => {
  ...
  const toggleRenewMode = () => {
    ...
    setUserFound(false);
    setPhoneNumber("");
    // Reset the selected plan when toggling to renewal mode
  ...
  const searchSubscription = async () => {
    ...
    try {
      const response = await fetch("/api/victor.php", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (data.status === true) {
        ...
        // Preencher os dados do cliente se a API retornar
        if (data.user) {
          setClientInfo({
            name: data.user.name || "",
      ...
      toast.error("Ocorreu um erro ao buscar sua assinatura. Por favor, tente novamente.");
    } finally {
          ...
          <div className="max-w-xl mx-auto bg-white rounded-xl shadow-md p-6 mb-10">
            ...
            <div className="flex space-x-2">
              ...
              <Button
                ...
                className="bg-whatsapp hover:bg-whatsappDark"
              >
